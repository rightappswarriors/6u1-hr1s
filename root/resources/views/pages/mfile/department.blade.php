								...
								<table class="table table-hover" id="dataTable">
									...
									<tbody>
										@isset($dept)
											@if(count($dept)>0)
												@foreach($dept as $pp)
												<tr data_id="{{$pp->deptid}}" data_name="{{$pp->dept_name}}">
													<td>{{$pp->deptid}}</td>
													<td>{{$pp->dept_name}}</td>
												</tr>
												@endforeach
...
@section('to-bottom')
	...
	<script type="text/javascript">
		...
		$('#opt-add').on('click', function() {
			$('#frm-pp').attr('action', '{{url('master-file/department')}}');
		...
		$('#opt-update').on('click', function() {
			$('#frm-pp').attr('action', '{{url('master-file/department')}}/update');
		...
		$('#opt-delete').on('click', function() {
			$('#frm-pp').attr('action', '{{url('master-file/department')}}/delete');
